...
def main():
    ...
    failures = []
    csv_paths = glob(
        "/Users/lucyford/Desktop/UOM CCL/Semester 2/Dissertation/Language dfs/*"
    )
    ...
    with open("MFA_all_symbols_corrected.txt", "w") as out:
        for symbol in sorted(all_symbols):
            print(f"{symbol}: {unicodedata.name(symbol)}", file=out)
    with open("MFA_parsed_phonemes_corrected.txt", "w") as out:
        out.writelines(sorted(all_phonemes_w_names))
    with open("MFA_parsing_failures_corrected.txt", "w") as out:
